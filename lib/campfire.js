  ...
  var request = this.http.request(options, function(response) {
    if (typeof(callback) == 'function') {
      ...
      response.on('end', function() {
        ...
        } catch(e) { console.log('error parsing response for '+path+': '+e+' ('+data+')') }
          ...
          for (var i = 0; i < chunk.length; ++i) {
            if (chunk[i] != '') {
              try {
                ...
                console.log('unable to parse response as JSON: '+chunk[i]);
...
Campfire.Room.prototype.get = function(subpath, callback) {
  this.campfire.get(this.path+subpath, callback);
};

Campfire.Room.prototype.post = function(subpath, body, callback) {
  this.campfire.post(this.path+subpath, body, callback);
};
